...
import common.db.base.jpa.annotation.FieldDisplayName;
...
public class EntityBeanUtil {
		...
		for (Field field : fields) {
			...
			for (PropertyDescriptor property : propertyDescriptors) {
				if (fieldname.equals(property.getName())) {
					...
					if (t == null) {
						if (reader != null && !(isLazyField(reader.getAnnotations()))) {
	...
	public String beanToString() {
		...
		for (String fieldname : fieldnames) {
			...
			sb_tostring.append(getFieldDisplayName(fieldname)).append(":[").append(obj_value).append("]\t");
		}
	...
	public static String getEnumDescription(Object emumValue) {
		...
		if (emumValue != null) {
			try {
				...
				desValue = emumValue.getClass().getField(enumName).getAnnotation(EnumDescription.class).value();
			} catch (Exception e) {
